...
import type { MarkNoShowCommand } from './mark-no-show.command';

const MILLISECONDS_PER_HOUR = 60 * 60 * 1000;

// RN-19: "un No-show se gestiona segun la misma politica de cancelacion tardia" - reusa
// CancellationPolicy, no una NoShowPolicy propia (no existe en el catalogo de
// CompanySettings).
@Injectable()
export class MarkNoShowHandler {
  ...
  async execute(command: MarkNoShowCommand): Promise<void> {
    ...
    const penalty = policy
      ? resolveCancellationPenalty(
          policy,
    ...
    reservation.markNoShow({ penalty, penaltyReason: 'CancellationPolicy (NoShow, RN-19)' });
