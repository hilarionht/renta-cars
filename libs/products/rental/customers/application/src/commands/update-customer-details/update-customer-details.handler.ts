...
import type { UpdateCustomerDetailsCommand } from './update-customer-details.command';

@Injectable()
export class UpdateCustomerDetailsHandler {
  constructor(
  ...
  async execute(command: UpdateCustomerDetailsCommand): Promise<void> {
    ...
    const contactInfo =
      command.contactEmail || command.contactPhone
        ? ContactInfo.from({
            email: command.contactEmail ?? customer.contactInfo.toEmail().toString(),
            phone: command.contactPhone ?? customer.contactInfo.toPhone().toString(),
          })
        : undefined;

    customer.updateDetails({
      name: command.name ? CustomerName.from(command.name) : undefined,
      contactInfo,
    });
