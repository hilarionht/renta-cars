...
import { StartMaintenanceHandler } from './start-maintenance.handler';
import type { VehicleRepository } from '../../ports/vehicle.repository';

const UNKNOWN_ID = '018e5a00-0000-7000-8000-000000000000';

function createVehicleWithScheduledMaintenance(): { vehicle: Vehicle; maintenanceId: string } {
...
function buildHandler(existingVehicle: Vehicle | null) {
  ...
  const handler = new StartMaintenanceHandler(vehicleRepository, unitOfWork);

  return { handler, vehicleRepository };
}

describe('StartMaintenanceHandler', () => {
  ...
  it('inicia el mantenimiento y persiste', async () => {
    const { vehicle, maintenanceId } = createVehicleWithScheduledMaintenance();
