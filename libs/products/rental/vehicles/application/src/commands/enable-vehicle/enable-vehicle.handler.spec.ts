...
function buildHandler(existingVehicle: Vehicle | null) {
  ...
  const handler = new EnableVehicleHandler(vehicleRepository, unitOfWork, eventPublisher);

  return { handler, vehicleRepository, eventPublisher };
}

describe('EnableVehicleHandler', () => {
  it('lanza VehicleNotFoundError si el vehicle no existe o es de otra company', async () => {
    const { handler } = buildHandler(null);

    await expect(
      handler.execute({ vehicleId: UNKNOWN_ID, companyId: 'company-1' }),
    ).rejects.toThrow(VehicleNotFoundError);
  });

  it('habilita el vehicle, persiste y publica VehicleEnabled.v1', async () => {
    const vehicle = createVehicleReadyToEnable();
    const { handler, vehicleRepository, eventPublisher } = buildHandler(vehicle);

    await handler.execute({ vehicleId: vehicle.id.toString(), companyId: 'company-1' });

    expect(vehicle.status).toBe('Available');
    expect(vehicleRepository.save).toHaveBeenCalledTimes(1);
    expect(eventPublisher.publish).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ eventType: 'VehicleEnabled.v1' }),
    );
  });
});
