...
export class PrismaSessionRepository implements SessionRepository {
  ...
  async findById(id: SessionId): Promise<Session | null> {
    const record = await this.prisma.session.findFirst({ where: { id: id.toString() } });
    return record ? this.toDomain(record) : null;
  }

  async findByRefreshTokenHash(hash: string): Promise<Session | null> {
    const record = await this.prisma.session.findFirst({ where: { refreshTokenHash: hash } });
    return record ? this.toDomain(record) : null;
  }

  async findActiveForUser(userId: string): Promise<Session[]> {
    const records = await this.prisma.session.findMany({ where: { userId, status: 'Active' } });
