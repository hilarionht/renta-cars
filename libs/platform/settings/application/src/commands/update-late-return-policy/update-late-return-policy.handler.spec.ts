...
import { UpdateLateReturnPolicyHandler } from './update-late-return-policy.handler';
...
function buildHandler(existingSettings: CompanySettings | null) {
  ...
  const handler = new UpdateLateReturnPolicyHandler(settingsRepository, unitOfWork, eventPublisher);

  return { handler, settingsRepository, eventPublisher };
}

describe('UpdateLateReturnPolicyHandler', () => {
  it('actualiza, persiste y publica CompanySettingsUpdated.v1', async () => {
    ...
    await handler.execute({
      companyId: 'company-a',
      graceMinutes: 15,
      penaltyPercentagePerHour: 20,
    });

    expect(settings.lateReturnPolicy.graceMinutes).toBe(15);
  ...
  it('lanza CompanySettingsNotFoundError si el repositorio no encuentra CompanySettings', async () => {
    ...
    await expect(
      handler.execute({ companyId: 'company-a', graceMinutes: 0, penaltyPercentagePerHour: 0 }),
    ).rejects.toThrow();
