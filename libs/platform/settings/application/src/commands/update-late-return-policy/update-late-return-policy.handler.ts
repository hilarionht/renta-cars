...
import { CompanySettingsNotFoundError, LateReturnPolicy } from '@platform/settings/domain';

import { SETTINGS_REPOSITORY, type SettingsRepository } from '../../ports/settings.repository';
import type { UpdateLateReturnPolicyCommand } from './update-late-return-policy.command';

@Injectable()
export class UpdateLateReturnPolicyHandler {
  constructor(
  ...
  async execute(command: UpdateLateReturnPolicyCommand): Promise<void> {
    ...
    settings.updateLateReturnPolicy(
      LateReturnPolicy.from({
        graceMinutes: command.graceMinutes,
        penaltyPercentagePerHour: command.penaltyPercentagePerHour,
      }),
    );
