...
import { ConfirmMfaEnrollmentHandler } from './confirm-mfa-enrollment.handler';
import type { ConfirmMfaEnrollmentCommand } from './confirm-mfa-enrollment.command';

function buildUser(companyId = 'company-1'): User {
  return User.create({
    companyId,
...
function buildHandler(overrides?: { existingUser?: User | null; codeIsValid?: boolean }) {
  ...
  return { handler, userRepository, eventPublisher };
}

const baseCommand: ConfirmMfaEnrollmentCommand = {
...
describe('ConfirmMfaEnrollmentHandler', () => {
  ...
  it('lanza UserNotFoundError si el usuario pertenece a otra company', async () => {
    const { handler } = buildHandler({ existingUser: buildUser('company-2') });

    await expect(handler.execute(baseCommand)).rejects.toThrow(UserNotFoundError);
  });

  it('lanza InvalidMfaCodeError si el codigo no verifica contra el secret presentado', async () => {
    const { handler } = buildHandler({ codeIsValid: false });

    await expect(handler.execute(baseCommand)).rejects.toThrow(InvalidMfaCodeError);
  });

  it('en exito: cifra el secret, habilita MFA en el User y lo persiste', async () => {
    ...
    expect(saved.mfaSecret?.toString()).toBe('encrypted-secret');
  });
});
