...
Este directorio contiene procedimientos operativos para incidentes concretos — qué hacer cuando algo ya está pasando, no arquitectura ni diseño. Cada runbook asume que quien lo lee tiene acceso a producción/staging (o al entorno local equivalente) y sigue los pasos en orden. Excepción explícita: [06-diagnostico-recursos-maquina-de-desarrollo.md](06-diagnostico-recursos-maquina-de-desarrollo.md) cubre un incidente puramente local (la máquina de desarrollo, no producción/staging) — se incluye acá por seguir la misma estructura de 5 pasos, no por alcance.
...
| [06-diagnostico-recursos-maquina-de-desarrollo.md](06-diagnostico-recursos-maquina-de-desarrollo.md) | La máquina de desarrollo local se queda sin RAM (Docker + backend + bundler) y la sesión de trabajo se cuelga    |

## Cómo leer un runbook

Los 6 siguen la misma estructura:
