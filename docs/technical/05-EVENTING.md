...
- La emisión inmediata post-commit cubre el caso feliz con latencia mínima; el relay cubre el caso en que el proceso cayó entre el commit y la emisión, o un listener no estaba disponible — es la garantía _at-least-once_ que ADR-0005 ya reconoce como la garantía del bus, ahora extendida para sobrevivir a un reinicio de proceso (mejora explícita sobre el riesgo que ese ADR aceptó, sin introducir un broker externo, ver [10-DECISIONES.md](10-DECISIONES.md) #2).

## 3. Listeners

- Un módulo que reacciona a un evento de otro registra su `Listener` exclusivamente en su propia carpeta `infrastructure/events/` — nunca importa código del módulo emisor más allá del _shape_ versionado del evento ([05-CONVENCIONES-BACKEND.md §8](../05-CONVENCIONES-BACKEND.md)).
- El _shape_ de cada evento (tipo TypeScript del payload) se define junto al catálogo — una única fuente de verdad de tipos de evento, consumida tanto por el productor (para construir el payload) como por cualquier consumidor (para tipar su Listener), sin que el consumidor dependa del módulo productor más que de ese tipo.
...
- Los `Listener` con `@OnEvent` pasan a ser consumidores del broker (suscripción por tópico/stream) en lugar de manejadores locales — el _shape_ del evento, su validación (§4) y su disciplina de idempotencia (§3) no cambian, porque ya estaban diseñados para _at-least-once_ y entrega potencialmente duplicada.
